...
def main(Login, Password, Acc):
    ...
    #####BLCKS#####
    driver.get("https://wax.bloks.io/account/ancientrealm?loadContract=true&tab=Actions&account=ancientrealm&scope=ancientrealm&limit=100&action=usetool")
    ...
    ####MINERS####
    miner = driver.find_element(By.XPATH, "//input[@placeholder='Enter account name...']")
    miner.clear()
    miner.send_keys(Acc)
    timer = 1
    while timer < 20:
        timer += 1
        driver.find_element(By.XPATH, "//button[text()=' Submit Transaction ']").click()
        print("!!!Mined!!!")
